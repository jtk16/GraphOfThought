    ...
    def update_parameters(self, exps):
        ...
        for _ in range(self.epochs):
            ...
            for i in range(len(batch_indexes) // batch_size):
                ...
                ratio = torch.exp(log_probs - exps_batch.log_prob)
                surrogate1 = ratio * exps_batch.advantage
                surrogate2 = torch.clamp(ratio, 1.0 - self.clip_eps, 1.0 + self.clip_eps) * exps_batch.advantage
                policy_loss = -torch.min(surrogate1, surrogate2).mean()

                value_clipped = exps_batch.value + torch.clamp(value - exps_batch.value,
                                                                -self.clip_eps, self.clip_eps)
                value_loss = torch.max((value - exps_batch.returnn).pow(2),
                                       (value_clipped - exps_batch.returnn).pow(2)).mean()
