...
from kvtg.storage import KVTGStorage, CompressionMethod
...
class KVTGStorageBenchmark:
    ...
    def __init__(self, config: BenchmarkConfig):
        ...
        for method in config.compression_methods:
            self.storage_systems[method] = KVTGStorage(
                max_memory_items=10,
    ...
    def _measure_compression(self, tensor_shape: Tuple[int, ...], method: CompressionMethod) -> BenchmarkResult:
        """
        Measure compression performance for a single configuration.
        
        Args:
            tensor_shape: Shape of tensors to benchmark
            method: Compression method to test
            
        Returns:
            BenchmarkResult containing all measurements
        """
        storage = self.storage_systems[method]
    ...
    def run_comprehensive_benchmark(self) -> Dict[str, Any]:
        ...
        for i, tensor_shape in enumerate(self.config.tensor_configs):
            ...
            for j, method in enumerate(self.config.compression_methods):
                logger.info(f"  Method {j+1}/{len(self.config.compression_methods)}: {method.name}")
                
                try:
                    result = self._measure_compression(tensor_shape, method)
...
def create_standard_benchmark_config() -> BenchmarkConfig:
    ...
    compression_methods = [
        ...
        CompressionMethod.QUANTIZE_FP16,
        CompressionMethod.QUANTIZE_INT8,
